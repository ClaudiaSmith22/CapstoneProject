...
q_dot1new, q_dot2new = findQdot(fprime_t, f_g, r_dot1, r_dot2, f_t, fprime_g)
q_dot1=np.array([q_dot1new])
q_dot2=np.array([q_dot2new])
	...
	if (minimum-theta_min < 1e-20):
		...
		while m < len(allStart):
			if allStart[m]==1: #Update the ratio for selling banks  
				if m==0:
					newRatio=findRatioDeriv(allS_init[m], ratio1[-1], q_dot1, q[:,k], event1, psi1, r_dot1, dt)
				else:
					newRatio=findRatioDeriv(allS_init[m], ratio2[-1], q_dot2, q[:,k], event2, psi2, r_dot2, dt)
		...
		while ((limit-t[k])>dt_new):  #iterate until "whole" interval 
			...
			if c>0: #UPDATE IF ADDING BANKS
				...
				r_dot1= np.concatenate((r_dot1, [r_dot1[-1]]))
				r_dot2= np.concatenate((r_dot2, [r_dot2[-1]]))
				q_dot1new, q_dot2new = findQdot(fprime_t, f_g, r_dot1, r_dot2, f_t, fprime_g)
				q_dot1=np.concatenate((q_dot1, [q_dot1new]))
				q_dot2=np.concatenate((q_dot2, [q_dot2new]))
...
for row in q:
	...
	counter=counter+1
plt.title("Multi-Bank, Multi-Asset Method 2 Results")
plt.xlabel("Time (t)")
...
plt.legend(handles=[plum1_patch, plum2_patch, orange_patch, blue_patch, red_patch, green_patch])
plt.show()
plt.figure(2)
plt.plot(tnew, allTheta[0], color='orange') 
plt.plot(tnew, allTheta[1], color='blue') 
plt.plot(t,-ratio1, color='red')  
plt.plot(t,-ratio2, color='green')		
plt.title("Multi-Bank, Multi-Asset Method 2 Selling and Capital Ratio Results")
plt.xlabel("Time (t)")
plt.ylabel("Scaled Simulation Value")
